...
const BottomMenu = ({ className }) => {
  ...
  const activeItem = useMemo(
    () =>
      [...bottomMenuItem, profileMenuItem].find((item) =>
        isMenuItemActive(location.pathname, item),
      ),
    [location.pathname],
  );
    ...
    <>
      ...
      >
        ...
        >
          ...
          <li
            ref={(node) => {
              itemRefs.current[profileMenuItem.id] = node;
            }}
            className="relative z-10 mt-auto block w-full"
          >
            <NavLink
              to={profileMenuItem.to}
              title={profileMenuItem.label}
              onPointerDown={captureActiveIndicatorPosition}
              onClick={() => setMobileDrawerOpen(false)}
              className={({ isActive }) =>
                `group flex h-16 w-full items-center bg-white/12 hover:bg-white/10 justify-start gap-4 rounded-l-full rounded-r-none pl-4 pr-0 text-white/70 transition-colors duration-300
                ${isActive ? "bg-transparent" : "hover:bg-white/10 md:hover:bg-transparent"}`
              }
            >
              {({ isActive }) => (
          ...
          {/* <li className="relative z-10 w-full pb-4 pt-2">
            <button
