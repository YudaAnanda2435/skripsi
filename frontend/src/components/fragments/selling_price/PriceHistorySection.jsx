...
const PriceHistorySection = ({
  ...
  const [jumlahTampil, setJumlahTampil] = useState(HISTORY_ITEMS_PER_PAGE);
  const riwayatTerlihat = useMemo(
    () => riwayatHargaTampil.slice(0, jumlahTampil),
    [jumlahTampil, riwayatHargaTampil],
  );
  const masihAdaRiwayat = jumlahTampil < riwayatHargaTampil.length;

  useEffect(() => {
    setJumlahTampil(HISTORY_ITEMS_PER_PAGE);
  }, [riwayatHargaTampil]);
    ...
    <section className="overflow-hidden rounded-xl border border-[#e5e2da] bg-white shadow-[0_4px_24px_rgba(23,61,45,0.04)]">
      ...
      {riwayatHargaTampil.length === 0 ? (
        ...
        <>
          ...
          {masihAdaRiwayat && (
            <div className="border-t border-[#e5e2da] bg-white px-5 py-4 text-center">
              <button
                type="button"
                onClick={() =>
                  setJumlahTampil((jumlah) => jumlah + HISTORY_ITEMS_PER_PAGE)
                }
